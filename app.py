...
if menu == "Ver mi quiniela guardada":
    ...
    if st.button("Consultar"):
        ...
        if data.get("status") == "success":
            ...
            for etapa, contenido in fases.items():
                with st.expander(f"Ver {etapa}"):
                    if isinstance(contenido, dict):
                        for p, det in contenido.items(): st.write(f"**{p}:** {det}")
                    else: st.write(contenido)
    ...
    if nombre:
        ...
        octavos_manual = st.text_area("Partidos de Octavos (Ej: O1: México vs Suiza):")
        campeon = st.text_input("Campeón Final:")
        
        if st.button("🚀 ENVIAR QUINIELA"):
            # Construimos el payload con los resultados de grupos + octavos
            payload = {
                "nombre": nombre, "estado": "En Edición", "campeon": campeon,
                "octavos": json.dumps({"Octavos": octavos_manual}, ensure_ascii=False),
